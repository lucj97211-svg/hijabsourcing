...
export default function GsmSlider() {
  ...
  return (
    <div className="studio-block" data-component="studio-gsm-slider">
      <div className="studio-block__head">
        <span className="mono studio-step">01 — Weight</span>
        <h3>Set the fabric weight</h3>
        <p className="muted studio-block__hint">
          Drag to change the weight. The preview thickens and closes up as the fabric gets heavier.
        </p>
      </div>
      ...
      <p className="gsm-note" role="status">
        {zone.note}
      </p>
      ...
      <p className={`mono gsm-band ${inBand ? "is-ok" : "is-warn"}`}>
        {inBand
          ...
          : `Outside the standard ${fabric.name} band (${fabric.gsm[0]}–${fabric.gsm[1]} GSM) — ask us if it can be run`}
      </p>
    </div>
  );
}
